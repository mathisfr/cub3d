...
static void	check_borders(t_map *map)
{
	...
	while (++x < map->max_x_size)
		if ((map->map_arr[0][x] != ' ' && (map->map_arr[0][x] != '1' && map->map_arr[0][x] != '2'))
			|| (map->map_arr[map->max_y_size - 1][x] != ' '
				&& (map->map_arr[map->max_y_size - 1][x] != '1' && map->map_arr[map->max_y_size - 1][x] != '2')))
			parsing_error("Map is not enclosed");
	x = -1;
	while (++x < map->max_y_size)
		if ((map->map_arr[x][0] != ' ' && map->map_arr[x][0] != '1' && map->map_arr[x][0] != '2')
			|| (map->map_arr[x][map->max_x_size - 1] != ' '
				&& (map->map_arr[x][map->max_x_size - 1] != '1' && map->map_arr[x][map->max_x_size - 1] != '2')))
			parsing_error("Map is not enclosed");
}
...
{
	if (y != 0)
		if (map->map_arr[y - 1][x] != ' ' && (map->map_arr[y - 1][x] != '1' && map->map_arr[y - 1][x] != '2'))
			parsing_error("Map is not enclosed");
	if (y < map->max_y_size - 1)
		if (map->map_arr[y + 1][x] != ' ' && (map->map_arr[y + 1][x] != '1' && map->map_arr[y + 1][x] != '2'))
			parsing_error("Map is not enclosed");
	if (x != 0)
		if (map->map_arr[y][x - 1] != ' ' && (map->map_arr[y][x - 1] != '1' && map->map_arr[y][x - 1] != '2'))
			parsing_error("Map is not enclosed");
